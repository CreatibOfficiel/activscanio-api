...
export class AddGamificationSystem1764269630000 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."IDX_bettor_rankings_streaks"`,
    );
    await queryRunner.query(
      `ALTER TABLE "bettor_rankings" DROP COLUMN "consecutivePerfectBets"`,
    );
    await queryRunner.query(
      `ALTER TABLE "bettor_rankings" DROP COLUMN "perfectBetStreak"`,
    );
    await queryRunner.query(
      `ALTER TABLE "bettor_rankings" DROP COLUMN "weeklyParticipationStreak"`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN "lastAchievementUnlockedAt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN "achievementCount"`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "user_streaks" DROP CONSTRAINT IF EXISTS "FK_user_streaks_user"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."IDX_user_streaks_lifetime"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."IDX_user_streaks_monthly"`,
    );
    await queryRunner.query(`DROP TABLE "user_streaks"`);

    // Drop user_achievements table (with foreign keys)
    await queryRunner.query(
      `ALTER TABLE "user_achievements" DROP CONSTRAINT IF EXISTS "FK_user_achievements_achievement"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."IDX_achievements_rarity"`,
    );
    await queryRunner.query(
